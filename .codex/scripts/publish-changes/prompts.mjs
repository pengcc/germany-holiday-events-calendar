...
export async function confirmWithRetry({ ask, warning, message, maxInvalidAttempts = 2 }) {
  ...
  while (invalidAttempts < maxInvalidAttempts) {
    ...
    invalidAttempts += 1;
    warning(`Invalid response "${answer}". Enter y/yes or n/no; empty input defaults to no.`);
  }
...
export async function chooseValidation(prompts, classification, classificationPolicy) {
  ...
  const answer = (
    await prompts.ask(`Type validation code (${allowed.join(", ")}): `)
  ).toUpperCase();
...
export async function chooseCompletionMode(prompts, classification, classificationPolicy, output) {
  ...
  throw new PublishError("USER_CANCELLED", `Invalid completion mode "${answer || "<empty>"}".`);
}
