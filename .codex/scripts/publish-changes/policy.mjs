...
export function validatePolicy(policy) {
  ...
  for (const classification of CLASSIFICATIONS) {
    ...
    for (const key of POLICY_KEYS) {
      if (typeof values[key] !== "boolean") {
        throw new PublishError("MALFORMED_POLICY", `${classification}.${key} must be a boolean.`);
      }
    }
  }
  ...
  if (!policy.classifications.normal.require_validation) {
    throw new PublishError(
      "MALFORMED_POLICY",
      "Normal updates must require structured validation.",
    );
  }
  ...
  for (const classification of ["normal", "significant"]) {
    const values = policy.classifications[classification];
    if (
      (values.allow_auto_merge || values.allow_immediate_merge) &&
      !values.require_manual_review
    ) {
      throw new PublishError(
        "MALFORMED_POLICY",
...
export async function loadPolicy({ path, output, readFileImpl = readFile, yamlLoader } = {}) {
