...
export function createGitClient(commandRunner, cwd) {
  ...
  return {
    ...
    fetchDefault: (branch) => required(['fetch', 'origin', branch], `Could not fetch origin/${branch}`),
    upstream: async (branch) => {
      const result = await run(['rev-parse', '--abbrev-ref', '--symbolic-full-name', `${branch}@{upstream}`]);
      return result.ok ? result.stdout.trim() : '';
    },
    verifyRef: async (ref) => (await run(['rev-parse', '--verify', ref])).ok,
    logRange: (range, format = '--oneline') => required(['log', format, range], 'Could not inspect commits'),
    latestSubject: () => required(['log', '-1', '--format=%s'], 'Could not read latest commit subject'),
    diff: (args) => required(['--no-pager', 'diff', ...args], 'Could not inspect diff'),
    untracked: () => required(['ls-files', '--others', '--exclude-standard'], 'Could not inspect untracked files'),
    hashFiles: (paths) =>
      required(['hash-object', '--no-filters', '--', ...paths], 'Could not fingerprint untracked files'),
    includesDefault: async (defaultRef) => (await run(['merge-base', '--is-ancestor', defaultRef, 'HEAD'])).ok,
    addPaths: (paths) => required(['add', '-A', '--', ...paths], 'Could not stage confirmed changes'),
    ...
    switchCreate: (branch) => required(['switch', '-c', branch], `Could not create branch ${branch}`),
    ...
    createBackup: (branch) => required(['branch', branch], `Could not create backup branch ${branch}`),
