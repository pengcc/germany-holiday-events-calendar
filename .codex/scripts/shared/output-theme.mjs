...
export function validateOutputTheme(theme) {
  ...
  const unknownRoot = Object.keys(theme).filter(
    (key) => !['version', 'levels'].includes(key),
  );
  ...
  const unknownLevels = Object.keys(theme.levels).filter(
    (level) => !OUTPUT_LEVELS.includes(level),
  );
  ...
  for (const level of OUTPUT_LEVELS) {
    ...
    const unknownKeys = Object.keys(style).filter(
      (key) => !['color', 'fullLine'].includes(key),
    );
